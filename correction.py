...
if __name__=="__main__":
    ...
    if full_correction:
        map_pub = rospy.Publisher('tag_map/pose'+suffix, Odometry, queue_size=10)
    else:
        map_pub = rospy.Publisher('tag_map/pose'+suffix, PoseWithCovarianceStamped, queue_size=10)

    amcl_pub = rospy.Publisher('tag_map/amcl'+suffix, Odometry, queue_size=10)
    odom_pub = rospy.Publisher('tag_map/odom_filtered'+suffix, Odometry, queue_size=10)
    tag_pub = rospy.Publisher('tag_map/markers'+suffix, Vector3, queue_size=10)
    ...
    while not rospy.is_shutdown():
        ...
        try:
            check_tf = buf.lookup_transform('tag_world_cur'+suffix, 'base_link', rospy.Time(0), rospy.Duration(1))
            #amcl_tf = buf.lookup_transform('map', 'base_link', rospy.Time(0), rospy.Duration(1))
            #map_tf = buf.lookup_transform('tag_world_ref'+suffix, 'map', rospy.Time(0), rospy.Duration(1))
            #odom_tf = buf.lookup_transform('tag_world_ref'+suffix, 'base_link', rospy.Time(0), rospy.Duration(1))

            for tag in tag_list:
                location_list.append(buf.lookup_transform('tag_world_ref'+suffix, str(tag)+'_ref'+suffix, rospy.Time(0), rospy.Duration(1)))

            if full_correction:
                publish_odom(check_tf, map_pub) #Publish Tag_Pose
            else:
                publish_pose(check_tf, map_pub) #Publish Tag_Pose
